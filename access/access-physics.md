# GCSEs for Dads - Physics Access Guide

Welcome, and thanks for buying GCSEs for Dads - Physics. I hope this saves you time and helps your child get better grades! 

The topics are all online so follow the Phyics portal link and bookmark that. The links change every few months so if you need access later please just email me. 

## Your links

- [Physics portal](https://gcsesfordads.uk/physics-k7m4/)
- Support: stu@gcsesfordads.uk

## Notes

- These links are for personal use only.
- Please bookmark the main portal.
- If a link changes, contact support for the latest access.
- **Please don't share these links. It keeps the site alive if you can just share the link gcsesfordads.uk**